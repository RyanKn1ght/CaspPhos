...
<!-- Word count: 1377 including readme.md, excluding references -->
...
For previous work I compiled a database of inflammatory caspase substrates (currently unpublished data). As appreciation is developing for the more nuanced nature of caspase cleavage site consensus sequences, this report aims to provide a reproducible analysis and visualisation of features of caspase cleavage sites using R 4.0 [@R].
...
(ref:frequency-figure) Representations of phosphorylation prediction results (score threshold `r phosphate_threshold`) amongst a sample of `r substrates_in_deepphos` human inflammatory Caspase substrates. Position 10 is the cleavage site (D/E) A) Plot of prevelance of possible phosphorylated residues within the data set. B) Evaluation of the strength of mean phosphate prediction scores for each position. Error bars show the standard error of the mean.
...
tsne <- dummy_format %>% 
  Rtsne(perplexity = 5, check_duplicates = FALSE)
...
Unexpectedly, tSNE results consistently demonstrate two separate populations, one of caspase-1 substrates, and one of Caspase-4 and 5 susbtrates. Caspase-4 and 5 have long been considered to play similar roles, as the human orthologues of murine Caspase-11.

The primary sequence of cleavage sites alone is sufficient to separate inflammatory caspase substrates, without taking into account amino acid chemistry and post-translational modification. Combined with the lack of an evident single consensus pattern based on amino-acid identity or chemistry (Figures \@ref(fig:consensus-fig) and \@ref(fig:characteristics-fig)), this suggests the existence of complex patterns in the local amino acid sequence of inflammatory caspase cleavage sites that confer their specificity.
