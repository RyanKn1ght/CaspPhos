...
(ref:generic-cons-fig) Visualisation of inflammatory Caspase cleavage sites consensus categorised by chemistry. Top left: all Caspases (x), top right: Caspase-4 (y), bottom left: human Caspase-1, bottom right: murine Caspase-1.

<!-- End references segement -->

The acquired database of inflammatory caspase substrates was tidied into an amenable format, and cleavage site consensus interpreted with ggseqlogo [@wagih_2017]. Amino acids were categorised by chemical properties- acidity/basicity, hydrophobicity/polarity, and visualised in four plots: all inflammatory caspase substrates (x), human Caspase-1 substrates (y), murine Caspase-1 substrates (z) and the human-exclusive Caspase-4 substrates (a). Human Caspase-5 and murine Caspase-11 substrate consensus is not presented as few substrates have been recorded. Visualisations are shown in Figure \@ref(fig:consensus-fig).

```{r consensus-fig, fig.width=6, fig.cap = "(ref:generic-cons-fig)"}

#Categorise plots to represent in consensus sequences.
casp1_H_data <- sequence_table %>% filter(cleaved_by == 1 & species == "H")
casp1_M_data <- sequence_table %>% filter(cleaved_by == 1 & species == "M")
...
casp1_H <- ggseqlogo(casp1_H_data$sequence, seq_type = "aa", method = "prob")
casp1_M <- ggseqlogo(casp1_M_data$sequence, seq_type = "aa", method = "prob")
casp4 <- ggseqlogo(casp4_data$sequence, seq_type = "aa", method = "prob")

#Present consensus sequence plots in grid.
grid.arrange(all_casp, casp4, casp1_H, casp1_M, ncol=2)
